...
- `responsibilities`: Bounded work the agent may perform.
- `prohibited_actions`: Actions the agent must not perform.
- `token_analysis`: Required phase recording instruction.
- `inputs`: Expected artifacts or user inputs.
- `outputs`: Expected artifacts or reports.
- `stop_conditions`: Conditions that require stopping or asking the user.
